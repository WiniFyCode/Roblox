...
----------------------------------------------------------
-- 🔹 TrigerSkill GunFire Dupe Functions
local trigerSkillDupeEnabled = true
local trigerSkillDupeCount = 5 -- số lần gửi mỗi khi game gửi 1 gói

local oldTrigerSkillNamecall
if hookmetamethod and getnamecallmethod and checkcaller then
	oldTrigerSkillNamecall = hookmetamethod(game, "__namecall", function(remoteInstance, ...)
		...
		if trigerSkillDupeEnabled
			...
			if firstArgument == "GunFire" and secondArgument == "Atk" then
				for iterationIndex = 1, trigerSkillDupeCount do
	...
	warn("[ZombieHyperloot] Executor không hỗ trợ hookmetamethod/getnamecallmethod/checkcaller - TrigerSkill dupe tắt")
end
...
CombatTab:AddToggle("TrigerSkillDupeEnabled", {
    ...
    Description = "Bật/tắt việc dupe gói GunFire.Atk để gây thêm sát thương",
    Default = trigerSkillDupeEnabled,
    Callback = function(Value)
        trigerSkillDupeEnabled = Value
        if trigerSkillDupeEnabled then
            print("[ZombieHyperloot] TrigerSkill dupe được bật")
        else
            print("[ZombieHyperloot] TrigerSkill dupe bị tắt")
        end
    end
})

CombatTab:AddSlider("TrigerSkillDupeCount", {
    Title = "TrigerSkill Dupe Count",
    Description = "Số lần gửi mỗi gói GunFire.Atk (càng cao càng mạnh nhưng dễ lag)",
    Default = trigerSkillDupeCount,
    Min = 1,
    Max = 20,
    Rounding = 0,
    Callback = function(Value)
        trigerSkillDupeCount = Value
        print("[ZombieHyperloot] TrigerSkill dupe count:", trigerSkillDupeCount)
    end
})
